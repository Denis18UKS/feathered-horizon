...
app.post('/subscribe', (req, res) => {
    ...
    if (!user_id_1 || !user_id_2) {
        return res.status(400).json({ message: "ID пользователя обязательно" });
    }

    // Проверим, подписан ли пользователь уже
    const checkQuery = `
    SELECT * FROM subscribes 
    WHERE (user_id_1 = ? AND user_id_2 = ?) 
    OR (user_id_1 = ? AND user_id_2 = ?)
    `;
    db.execute(checkQuery, [user_id_1, user_id_2, user_id_2, user_id_1], (err, results) => {
        if (err) {
            console.error('Ошибка при проверке подписки: ', err);
            return res.status(500).json({ message: "Ошибка на сервере" });
        }

        // Если подписка уже существует, обновляем поле isSubscribe на true
        if (results.length > 0) {
            const updateQuery = `
            UPDATE subscribes 
            SET isSubscribe = true, updated_at = CURRENT_TIMESTAMP 
            WHERE (user_id_1 = ? AND user_id_2 = ?) 
            OR (user_id_1 = ? AND user_id_2 = ?)
            `;
            db.execute(updateQuery, [user_id_1, user_id_2, user_id_2, user_id_1], (err, result) => {
                if (err) {
            ...
            // Если подписки еще нет, добавляем новую запись
            const insertQuery = `INSERT INTO subscribes (user_id_1, user_id_2, isSubscribe) VALUES (?, ?, true)`;
            db.execute(insertQuery, [user_id_1, user_id_2], (err, result) => {
                if (err) {
...
app.post('/unsubscribe', (req, res) => {
    ...
    // Обновляем статус подписки на false (отписка)
    const updateQuery = `
    UPDATE subscribes
    SET isSubscribe = false
    WHERE user_id_2 = ? AND user_id_1 = ?
    `;
    db.execute(updateQuery, [user_id_2, req.user.id], (err, results) => {
        if (err) {
            console.error('Ошибка при отписке: ', err);
            return res.status(500).json({ message: "Ошибка на сервере" });
        }

        return res.status(200).json({ message: "Вы отписались от пользователя" });
    });
});
